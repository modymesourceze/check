...
SUDO = int(os.environ.get("SUDO","6581896306"))
...
def start(message):
	...
	if f == SUDO:
		...
		M = types.InlineKeyboardButton('DEV', url='https://t.me/elhyba')
		mas.add(G,K)
		mas.add(V)
		mas.add(M)
		...
		me = types.InlineKeyboardButton(text="▶ المطور ◀",url="https://t.me/elhyba")
		he = types.InlineKeyboardButton(text="▶ قناة الملفات ◀",url="https://t.me/UP_UO")
		de = types.InlineKeyboardButton(text="▶ جروب الدعم ◀",url="https://t.me/ZeSupport")
		ch = types.InlineKeyboardButton(text="▶ قناة البوت ◀",url="https://t.me/Source_Ze")
		rr.add(me,he,de)
		rr.add(ch)
...
def masg(call):
	...
	if call.data =="elhyba":
		...
		while True:
			...
			elhyba = [h1,h2,h3,h4,h5]
			j = random.choice(elhyba)
			url = "https://t.me/"+j
			headers = {
            "User-Agent": generate_user_agent(),
            "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8",
			...
			if response.text.find('If you have <strong>Telegram</strong>, you can contact <a class="tgme_username_link"')>=0 and j not in ban:
				...
				R = types.InlineKeyboardButton(f'{sk}', callback_data="1x")
				elhyba = types.InlineKeyboardButton('DEV', url='https://t.me/elhyba')
				mas.add(A,E,B,R,elhyba)
				bot.edit_message_text(chat_id=call.message.chat.id,message_id=call.message.message_id,text="ok start",reply_markup=mas)
